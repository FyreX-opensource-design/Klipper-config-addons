...
if float(A) + float(B) + float(C) + float(D) > 1:
    os.system("echo M118 values are greater than 1")

if float(A) or float(B) or float(C) + float(D) == 1:
    ...
    os.system("echo SYNC_EXTRUDER_MOTION EXTRUDER=extruder3 MOTION_QUEUE=")
    if float(A) == 1:
        ...
        os.system("echo SET_EXTRUDER_ROTATION_DISTANCE EXTRUDER=extruder3 DISTANCE=0 > tmp/printer")
    elif float(B) == 1:
        ...
        os.system("echo SET_EXTRUDER_ROTATION_DISTANCE EXTRUDER=extruder3 DISTANCE=" + extruder3_rot + " > tmp/printer")
        os.system("echo SET_EXTRUDER_ROTATION_DISTANCE EXTRUDER=extruder DISTANCE=0 > tmp/printer")
        os.system("echo SET_EXTRUDER_ROTATION_DISTANCE EXTRUDER=extruder1 DISTANCE=0 > tmp/printer")
        os.system("echo SET_EXTRUDER_ROTATION_DISTANCE EXTRUDER=extruder2 DISTANCE=0 > tmp/printer")
else:
    ...
    os.system("echo SET_EXTRUDER_ROTATION_DISTANCE EXTRUDER=extruder1 DISTANCE=" + extruder4_rot / (float(D) * 10) + " > tmp/printer")
    ...
    os.system("echo SYNC_EXTRUDER_MOTION EXTRUDER=extruder3 MOTION_QUEUE=mixing")